...
: width(_width), height(_height), depth(_depth), sp(0)
{
	...
	// tirar isto para desfocar a imagem para a resolucao correta ou meter linear no final
...
void TextureArray::addTexture(const char path[]) {
	...
	if (_width != this->width || _height != this->height) {
		fprintf(stderr, "Error with image dimensions in %s. Expected %d %d got %d %d\n", __func__, this->width, this->height, _width, _height);
		exit(EXIT_FAILURE);
	}
