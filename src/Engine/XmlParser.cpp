...
Engine_Object_Materials XmlParser::parseEngineObjectMaterials(tinyxml2::XMLElement *model) {
	glm::vec3 diffuse = glm::vec3(200 / 255.0f, 200 / 255.0f, 200 / 255.0f);
	glm::vec3 ambient = glm::vec3(50 / 255.0f, 50 / 255.0f, 50 / 255.0f);
...
PointLight XmlParser::parsePointLight(tinyxml2::XMLElement *light) {
    ...
    if (light != nullptr) {
        ...
        constant = light->FloatAttribute("constant", 1.0f);
        linear = light->FloatAttribute("linear", 0.09f);
        quadratic = light->FloatAttribute("quadratic", 0.032f);
